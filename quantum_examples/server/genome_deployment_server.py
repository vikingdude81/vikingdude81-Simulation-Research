...
from evolution_engine.quantum_genetic_agents import QuantumAgent, load_genome
import numpy as np
import json
...
def list_genomes():
    ...
    # Security: only list files in current directory
    current_dir = Path.cwd()
    genome_pattern = str(current_dir / '*_genome.json')
    genome_files = glob.glob(genome_pattern)
    genome_info = []
    
    for filepath in sorted(genome_files):
        try:
            # Security: verify file is in current directory
            resolved_path = Path(filepath).resolve()
            if not str(resolved_path).startswith(str(current_dir)):
                continue  # Skip files outside current directory
...
def deploy_file(filename):
    ...
    try:
        ...
        # Only allow files in the current directory (no paths, no ..)
        ...
        # Resolve to absolute path and verify it's in the current directory
        current_dir = Path.cwd()
        genome_path = (current_dir / filename).resolve()
        
        # Ensure the resolved path is within the current directory
        if not str(genome_path).startswith(str(current_dir)):
            return jsonify({
                'status': 'error',
